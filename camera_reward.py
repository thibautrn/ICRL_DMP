...
# Set by SIGTERM from gazebo_exec when the DMP motion is done
_stop_capture = False

def _handle_stop(signum, frame):
    global _stop_capture
    print("📷 Stop signal received — finishing capture...")
    _stop_capture = True
...
# Hue is the tight discriminator (H_cv 91–110 = 182°–220°).
...
def get_rim_world_position(point_cloud, contour, frame_shape, n_top_rows=12, neighborhood=11):
    """
    Find the world position of the cup rim by sampling the topmost contour pixels.
...
def capture_trajectory(zed, duration, fps, video_path=None):
    import cv2
    import pyzed.sl as sl
    ...
    # Raw video writer (HD720 = 1280x720)
    video_writer = None
    if video_path is not None:
        fourcc       = cv2.VideoWriter_fourcc(*'mp4v')
        video_writer = cv2.VideoWriter(str(video_path), fourcc, fps, (1280, 720))
        if video_writer.isOpened():
            print(f"🎥 Recording raw video → {video_path}")
        else:
            print("⚠️  VideoWriter failed to open — skipping raw video")
            video_writer = None
    ...
    while not _stop_capture and time.time() - start_time < duration:
        now = time.time()
        ...
        if ball_pos is not None and cup_pos is not None:
            trajectory.append({
                ...
                'rim_pos':        list(rim_pos) if rim_pos is not None else None,
...
def capture_trajectory_mock(duration, fps):
    ...
    for i in range(n_frames):
        ...
        trajectory.append({
            ...
            'ball_pos':       ball_pos,
            'cup_pos':        cup_pos,
            'rim_pos':        [cup_pos[0], cup_pos[1], cup_pos[2] + 0.05],
            'cup_top_height': cup_pos[2] + 0.05,
            'above_cup':      ball_pos[2] > cup_pos[2] + 0.05,
            'ball_in_cup':    False,
        })
...
def compute_reward_from_trajectory(trajectory):
    ...
    # Use rim position for distance if available, fall back to cup centroid
    rim_positions  = np.array([
        t['rim_pos'] if t.get('rim_pos') is not None else t['cup_pos']
        for t in trajectory
    ])
    above_flags    = np.array([t['above_cup']    for t in trajectory])
    in_cup_flags   = np.array([t['ball_in_cup']  for t in trajectory])

    distances_3d = np.linalg.norm(ball_positions - rim_positions, axis=1)
...
def main():
    ...
    try:
        ...
        output_dir = Path("logs/video") / timestamp
        output_dir.mkdir(parents=True, exist_ok=True)

        if zed == "MOCK":
            trajectory = capture_trajectory_mock(CAPTURE_DURATION, CAPTURE_FPS)
        else:
            video_path = output_dir / f"raw_video_{iteration}.mp4"
            trajectory = capture_trajectory(zed, CAPTURE_DURATION, CAPTURE_FPS, video_path=video_path)
